...
async function buildOrderFilter(start, end, { hub, courier, awb, state, staffId, staffScope, department } = {}) {
  const f = { createdAt: { $gte: start, $lte: end } };
  if (department && department !== 'all') {
    f.department = String(department).toLowerCase();
  } else {
    f.department = { $in: ['male', 'ortho', 'skin'] };
  }
  f['order_items.name'] = { $not: /migraine|piles/i };
...
export async function getRtoReasons(params) {
  ...
  const normalize = (r) => {
    ...
    if (/refus|reject|cancel|not.?want/i.test(v))                return 'Customer Refused';
    ...
    if (/weather|flood|natural|delay/i.test(v))                  return 'Weather / Natural';
    return 'Other';
  };
  ...
  if (!Object.keys(map).length) {
    for (const r of srOrdersRto) { const k = normalize(r.status); map[k] = (map[k] || 0) + 1; }
    for (const r of smOrdersRto) { const k = normalize(r.status); map[k] = (map[k] || 0) + 1; }
  }
...
export async function getAlerts(params) {
  ...
  const { start, end } = buildDateRange(preset, from, to);
  const staffScope = params.staffId ? await getStaffScope(params.staffId) : null;
  const filter = await buildOrderFilter(start, end, { ...params, staffScope });
  const orders = await fetchOrderStats(filter);
  const kpis   = calcKPIs(orders);
  const alerts  = [];
  ...
  if (kpis.total > 0) {
    ...
    const rtoRate = +(((kpis.rto + kpis.rtoIntersite) / kpis.total) * 100).toFixed(1);
  ...
  return { alerts, kpis: { ndrRate: kpis.ndrRate, rtoRate: currentRtoRate, slaBreach } };
}
