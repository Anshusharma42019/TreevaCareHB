...
export const getDashboardStats = async (userRole, userId, targetDate, from, to, userDepartments = []) => {
  ...
  if (userDepartments && userDepartments.length > 0) {
    ...
    countFilter.department = { $in: ['male', 'ortho', 'skin'] };
    aggMatch.department = { $in: ['male', 'ortho', 'skin'] };
  }
  ...
  if (userDepartments && userDepartments.length > 0) {
    ...
    rtsAggMatch.department = { $in: ['male', 'ortho', 'skin'] };
  }
  ...
  const dateFilter = isAllTime ? {} : { createdAt: { $gte: start, $lte: end } };
  const updateDateFilter = isAllTime ? {} : { updatedAt: { $gte: start, $lte: end } };
  // departmentCountFilter always includes the date range so male/ortho/skin counts are period-accurate
  ...
  const [
    ...
    maleTaskToVerification,
    orthoTaskToVerification,
    skinTaskToVerification,
    malePendingVerifications,
    orthoPendingVerifications,
    skinPendingVerifications,
  ] = await Promise.all([
    Lead.countDocuments(countFilter),

    Lead.countDocuments({ ...countFilter, ...dateFilter }),

    Lead.countDocuments(departmentCountFilter('male')),

    Lead.countDocuments(departmentCountFilter('ortho')),

    Lead.countDocuments(departmentCountFilter('skin')),
    ...
    Verification.countDocuments({ ...countFilter, department: 'male', isDeleted: false, ...dateFilter }),
    Verification.countDocuments({ ...countFilter, department: 'ortho', isDeleted: false, ...dateFilter }),
    Verification.countDocuments({ ...countFilter, department: 'skin', isDeleted: false, ...dateFilter }),
    Verification.countDocuments({ ...countFilter, department: 'male', status: 'pending', isDeleted: false }),
    Verification.countDocuments({ ...countFilter, department: 'ortho', status: 'pending', isDeleted: false }),
    Verification.countDocuments({ ...countFilter, department: 'skin', status: 'pending', isDeleted: false }),
  ]);
  ...
  const [todayClosedLost, maleConverted, orthoConverted, skinConverted] = await Promise.all([
    Lead.countDocuments({ ...countFilter, status: 'closed_lost', ...updateDateFilter }),
    Verification.countDocuments(verifDeptFilter('male')),
    Verification.countDocuments(verifDeptFilter('ortho')),
    Verification.countDocuments(verifDeptFilter('skin')),
  ]);
  ...
  const buildDeliveredMatch = () => {
    ...
    const deptCond = (userDepartments && userDepartments.length > 0)
      ? { department: { $in: userDepartments } }
      : { department: { $in: ['male', 'ortho', 'skin'] } };

    if (isAllTime) return { status: { $in: statusList }, ...deptCond };
    ...
    return { 
      ...
      $or: [
        ...
        { $and: [{ $or: [{ delivered_at: null }, { delivered_at: { $exists: false } }] }, { status_updated_at: { $gte: start, $lte: end } }] }
      ]
    };
  };
  ...
  const [orderBreakdownSR, deliveredBreakdownSR, blSR, orderBreakdownSM, deliveredBreakdownSM, blSM, opsMonthData, smxInTransitCount, smxOfdCount] = await Promise.all([
    ...
    ShipmaxxOrder.countDocuments({ status: { $in: ['IN_TRANSIT', 'IN TRANSIT', 'INT', 'in_transit'] }, ...(userDepartments?.length > 0 ? { department: { $in: userDepartments } } : { department: { $in: ['male', 'ortho', 'skin'] } }) }),
    ShipmaxxOrder.countDocuments({ status: { $in: ['OUT_FOR_DELIVERY', 'OUT FOR DELIVERY', 'OFD', 'out_for_delivery'] }, ...(userDepartments?.length > 0 ? { department: { $in: userDepartments } } : { department: { $in: ['male', 'ortho', 'skin'] } }) })
  ]);
  ...
  const departmentLeads = {
    male: maleLeadCount,
    ortho: orthoLeadCount,
    skin: skinLeadCount,
    total: maleLeadCount + orthoLeadCount + skinLeadCount,
  };

  const maleConversionRate = maleLeadCount > 0 ? Math.round((maleConverted / maleLeadCount) * 100) : 0;
  const orthoConversionRate = orthoLeadCount > 0 ? Math.round((orthoConverted / orthoLeadCount) * 100) : 0;
  const skinConversionRate = skinLeadCount > 0 ? Math.round((skinConverted / skinLeadCount) * 100) : 0;
