...
export const downloadManifestHtml = async (awb) => {
  try {
    return await call('GET', `/shipping/manifest/${awb}`, { responseType: 'arraybuffer' });
  } catch (e1) {
    try {
      return await call('GET', '/shipping/download-manifest', { params: { awb }, responseType: 'arraybuffer' });
    } catch (e2) {
      try {
        return await call('POST', '/shipping/manifest', { data: { awb: String(awb), order_id: String(awb) }, responseType: 'arraybuffer' });
      } catch (e3) {
        throw e1;
      }
    }
