...
const safeCreateSmxOrder = async (payload) => {
  let attempts = 0;
  while (attempts < 5) {
    try {
      return await smx.createOrder(payload);
    } catch (err) {
      const errStr = String(err.message || '') + ' ' + String(err.stack || '') + ' ' + JSON.stringify(err.response?.data || '');
      const isConflict =
        err.statusCode === 409 ||
        err.response?.status === 409 ||
        errStr.includes('409') ||
        errStr.toLowerCase().includes('conflict') ||
        errStr.toLowerCase().includes('already exist') ||
        errStr.toLowerCase().includes('duplicate');

      if (isConflict && attempts < 4) {
        attempts++;
        const randomStr = Math.floor(1000 + Math.random() * 9000);
        const baseNum = String(payload.order_number).replace(/-R\d+$/, '');
        payload.order_number = `${baseNum}-R${randomStr}`;
        console.log(`[ShipMaxx] Order ID conflict detected. Retrying attempt ${attempts} with ID: ${payload.order_number}`);
      } else {
        throw err;
      }
    }
...
export const createOrder = catchAsync(async (req, res) => {
  ...
  const data = await safeCreateSmxOrder(payload);
  const smxRes = data?.data || data || {};
  const oid = smxRes.order_id || smxRes.id || payload.order_number;
  ...
  try {
    ...
    let orderDept = req.body.department || null;
    if (!orderDept && matchedLeadId) {
      const lead = await Lead.findById(matchedLeadId).select('department').lean();
      if (lead?.department) orderDept = lead.department;
    }
    if (!orderDept && req.user?.departments?.[0]) {
      const userDept = String(req.user.departments[0]).toLowerCase();
      if (['male', 'ortho', 'skin'].includes(userDept)) orderDept = userDept;
    }
    if (!orderDept) {
      const prodStr = (products || []).map(p => (p.name || '').toLowerCase()).join(' ');
      if (prodStr.includes('ortho')) orderDept = 'ortho';
      else if (prodStr.includes('skin')) orderDept = 'skin';
      else orderDept = 'male';
    }
...
export const getManifest = catchAsync(async (req, res) => {
  ...
  let buffer;
  try {
    buffer = await smx.downloadManifestHtml(awb);
  } catch (err) {
    console.error(`[ShipMaxx Manifest Download Error for ${awb}]:`, err.message);
    return res.status(400).json(new ApiResponse(400, null, err.message || 'Manifest not available yet from ShipMaxx'));
  }
  ...
  const startStr = buffer.toString('utf8', 0, 50).trim().toLowerCase();
  
  // Handle HTML manifest documents
  if (startStr.startsWith('<!doctype') || startStr.startsWith('<html') || startStr.startsWith('<div') || startStr.startsWith('<table')) {
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    return res.send(buffer);
  }

  // Handle PDF manifest documents
  if (buffer.toString('utf8', 0, 20).trim().startsWith('%PDF-')) {
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `inline; filename="manifest-${awb}.pdf"`);
    return res.send(buffer);
  }

  // Check if JSON error returned
  const fullText = buffer.toString('utf8');
  try {
    const json = JSON.parse(fullText);
    return res.status(400).json(new ApiResponse(400, null, json.message || json.detail || 'Manifest not available yet'));
  } catch (e) {
    if (fullText.toLowerCase().includes('<html') || fullText.toLowerCase().includes('<table') || fullText.toLowerCase().includes('<body')) {
      res.setHeader('Content-Type', 'text/html; charset=utf-8');
      return res.send(buffer);
    }
...
export const getOrders = catchAsync(async (req, res) => {
  ...
  const [orders, total] = await Promise.all([
    Order.find(match)
      .populate({ path: 'lead_id', select: 'phone email assignedTo department problem', populate: { path: 'assignedTo', select: 'name role' } })
      .populate('verified_by', 'name role')
      .populate('comments.createdBy', 'name role')
  ...
  orders.forEach(o => {
    if (!o.problem && o.lead_id?.problem) {
      o.problem = o.lead_id.problem;
    }
    if (!o.department) {
      if (o.lead_id?.department) {
        o.department = o.lead_id.department;
      } else {
        const prodStr = (o.order_items || []).map(p => (p.name || '').toLowerCase()).join(' ');
        if (prodStr.includes('ortho')) o.department = 'ortho';
        else if (prodStr.includes('skin')) o.department = 'skin';
        else o.department = 'male';
      }
    }
...
const detectOrderDepartment = (order) => {
  ...
  const skinRegex = /\b(skin|acne|pimple|rash|eczema|psoriasis|derma|pigmentation|face)\b/i;

  if (maleRegex.test(text)) return 'male';
  if (orthoRegex.test(text)) return 'ortho';
  if (skinRegex.test(text)) return 'skin';
...
export const getCompletedFollowUps = catchAsync(async (req, res) => {
  ...
  if (department && department !== 'all') {
    conditions.push({ $or: [{ department }, { department: null }, { department: { $exists: false } }] });
  } else {
    conditions.push({ $or: [{ department: { $in: ALLOWED_DEPTS } }, { department: null }, { department: { $exists: false } }] });
  }
...
export const sendToVerification = catchAsync(async (req, res) => {
  ...
  const task = await Task.create({
    ...
    department: lead.department || oldVer?.department || 'male',
  });
...
export const backfillDepartments = catchAsync(async (req, res) => {
  ...
  // 1. Fetch all leads with department
  const leads = await Lead.find({ department: { $in: ['male', 'ortho', 'skin'] } }).select('_id department phone').lean();
  const leadDeptMap = new Map();
  const phoneDeptMap = new Map();
  leads.forEach(l => {
    leadDeptMap.set(String(l._id), l.department);
    if (l.phone) {
      const p = String(l.phone).replace(/\D/g, '').slice(-10);
      if (p.length === 10) phoneDeptMap.set(p, l.department);
    }
  });

  // 2. Fetch all staff with departments
  const users = await User.find({ departments: { $elemMatch: { $in: ['male', 'ortho', 'skin'] } } }).select('_id departments').lean();
  const userDeptMap = new Map();
  users.forEach(u => {
    if (u.departments?.[0]) userDeptMap.set(String(u._id), u.departments[0]);
  });

  // 3. Bulk update ShipmaxxOrders
  const smxOrders = await ShipmaxxOrder.find({}).select('_id lead_id created_by verified_by task_created_by billing_phone department').lean();
  const smxBulk = [];
  let smxUpdated = 0;
  let smxReset = 0;

  for (const o of smxOrders) {
    let dept = null;
    ...
    if (dept !== o.department) {
      ...
      if (dept) smxUpdated++; else smxReset++;
    }
  }
