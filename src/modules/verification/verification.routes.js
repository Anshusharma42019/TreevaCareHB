...
router.get('/', auth('admin', 'manager', 'sales', 'support', 'logistics', 'doctor', 'staff'), departmentFilter, async (req, res) => {
  try {
    ...
    if (deptUpdates.length > 0) {
      deptUpdates.forEach(r => {
        const dept = r.assignedTo?.departments?.[0] || r.lead?.department || r.task?.department || 'male';
        r.department = dept;
...
router.get('/test-data', async (req, res) => {
  ...
  for (const r of records) {
    if (!r.department) {
      let dept = 'male'; // aggressive fallback
      if (r.assignedTo && r.assignedTo.departments && r.assignedTo.departments.length > 0) {
  ...
  try {
    ...
    if (deptUpdates.length > 0) {
      await Promise.all(deptUpdates.map(r => {
        const dept = r.assignedTo?.departments?.[0] || r.lead?.department || 'male';
        r.department = dept;
